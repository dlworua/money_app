...
class EnhancedAiCoach {
  ...
  Future<ComprehensiveInsight> generateComprehensiveInsight(
    ...
    try {
      ...
      final goalSuggestions = _analysisService.generateSavingGoals(transactions, budgets, 0.1);
      ...
      // 4. 예측 모델링
      final predictions = _generatePredictions(user, transactions, budgets, spendingAnalysis);
      
      // 5. 개인화 전략
      final strategies = _generatePersonalizedStrategies(
        user, transactions, budgets, spendingAnalysis, behaviorPatterns, psychProfile
      );
      
      // 6. 맞춤 메시지 생성
      final message = _generateDetailedMessage(
        user, spendingAnalysis, budgetAnalysis, behaviorPatterns, 
        psychProfile, predictions, strategies
      );
  ...
  /// 📊 행동 패턴 분석
  Map<String, dynamic> _analyzeBehaviorPatterns(List<Transaction> transactions) {
  ...
  /// ⏰ 시간적 패턴 분석
  Map<String, dynamic> _analyzeTemporalPatterns(List<Transaction> transactions) {
    final patterns = <String, dynamic>{};
    ...
    // 월중 지출 패턴 (급여일 기준)
    final monthlyPhases = <String, double>{
    ...
    patterns['dayOfWeek'] = {
      ...
      'peakDay': dayOfWeekSpending.entries.isNotEmpty 
          ? dayOfWeekSpending.entries.reduce((a, b) => a.value > b.value ? a : b).key
          : null,
    };
  ...
  /// 📈 카테고리 인사이트 분석
  Map<TransactionCategory, Map<String, dynamic>> _analyzeCategoryInsights(List<Transaction> transactions) {
    ...
    for (final category in TransactionCategory.values) {
      ...
      if (categoryTransactions.isNotEmpty) {
        ...
        final maxAmount = categoryTransactions.map((t) => t.amount).reduce(math.max);
        final minAmount = categoryTransactions.map((t) => t.amount).reduce(math.min);
        
        // 트렌드 분석 (최근 30일 vs 이전 30일)
        final now = DateTime.now();
        final recent = categoryTransactions.where((t) => 
          t.date.isAfter(now.subtract(const Duration(days: 30)))
        ).fold<double>(0, (sum, t) => sum + t.amount);
        
        final previous = categoryTransactions.where((t) => 
  ...
  /// 🧠 심리적 프로파일 생성
  Map<String, dynamic> _generatePsychologicalProfile(List<Transaction> transactions, UserModel user) {
    ...
    if (expenseTransactions.isNotEmpty) {
      // 리스크 성향 (큰 금액 거래 비율)
      final largeTransactions = expenseTransactions.where((t) => t.amount > 50000).length;
      final riskTolerance = largeTransactions / expenseTransactions.length;
      
      // 자제력 수준 (소액 빈번한 거래)
      final smallFrequent = expenseTransactions.where((t) => t.amount < 5000).length;
      final selfControlLevel = 1.0 - (smallFrequent / expenseTransactions.length).clamp(0.0, 1.0);
      ...
      profile['spendingPersonality'] = _determineSpendingPersonality(riskTolerance, selfControlLevel, planfulness);
    }
  ...
  Map<String, dynamic> _generatePredictions(
    ...
    final remainingDays = DateTime(DateTime.now().year, DateTime.now().month + 1, 1)
        .difference(DateTime.now()).inDays;
    
    predictions['monthEndSpending'] = spendingAnalysis['totalSpending'] + (dailyAvg * remainingDays);
    
    // 목표 달성 확률 (다중 요인 고려)
    final goalProbability = _calculateAdvancedGoalProbability(user, spendingAnalysis, transactions);
  ...
  List<PersonalizedStrategy> _generatePersonalizedStrategies(
    ...
    // 개인 성향 기반 전략
    final personality = psychProfile['spendingPersonality'] as String? ?? 'balanced';
    ...
    if (spendingAnalysis['topCategory'] != null) {
      ...
      strategies.add(PersonalizedStrategy(
        ...
        priority: 'HIGH',
        expectedSaving: ((spendingAnalysis['topCategoryAmount'] as double) * 0.2).round(),
        difficulty: '보통',
        timeframe: '2주',
      ));
    }
  ...
  String _generateDetailedMessage(
    ...
    if (spendingAnalysis['isEmpty'] != true) {
      ...
      if (spendingAnalysis['topCategory'] != null) {
        ...
        messages.add('최대 지출 카테고리: ${topCategory.displayName} (${_formatCurrency(topAmount)})');
      }
    }
    ...
    if (predictions['available'] == true) {
      final goalProb = ((predictions['goalAchievementProbability'] as double) * 100).round();
      messages.add('\\n🔮 **목표 달성 예측**');
    ...
    if (strategies.isNotEmpty) {
      ...
      for (int i = 0; i < math.min(3, strategies.length); i++) {
        ...
        messages.add('   예상 절약: ${_formatCurrency(strategy.expectedSaving.toDouble())}, 난이도: ${strategy.difficulty}');
      }
    }
  ...
  double _calculateSpendingConsistency(List<Transaction> transactions) {
    ...
    final mean = amounts.fold<double>(0, (sum, amount) => sum + amount) / amounts.length;
    final variance = amounts.fold<double>(0, (sum, amount) => sum + ((amount - mean) * (amount - mean))) / amounts.length;
  ...
  double _calculatePlanningScore(List<Transaction> transactions) {
    ...
    final weekdaySpending = transactions.where((t) => t.date.weekday <= 5)
        .fold<double>(0, (sum, t) => sum + t.amount);
    final weekendSpending = transactions.where((t) => t.date.weekday > 5)
        .fold<double>(0, (sum, t) => sum + t.amount);
  ...
  String _determineSpendingPersonality(double risk, double selfControl, double planfulness) {
  ...
  double _calculateConfidenceScore(List<Transaction> transactions, List<Budget> budgets) {
    double score = 0;
...
class ComprehensiveInsight {
  ...
  factory ComprehensiveInsight.empty(UserModel user) {
    return ComprehensiveInsight(
      ...
      strategies: [],
      personalizedMessage: '아직 분석할 데이터가 부족해요. 가계부를 더 작성해주시면 맞춤형 AI 조언을 드릴 수 있어요! 📝',
      actionableAdvice: [],
    );
  }
