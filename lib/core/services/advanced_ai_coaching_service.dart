...
class AdvancedAiCoachingService {
  ...
  Future<AiCoachingInsight?> getBudgetOverspendAlert(
    ...
    return AiCoachingInsight(
      ...
      message:
          '이달 ${categoryEnum.displayName} 예산 ${categoryData['budgetAmount'].toInt()}원을 '
      ...
      confidenceScore: 0.9,
      actionItems: _getSpecificAdviceForCategory(
        categoryEnum,
  ...
  Future<AiCoachingInsight> _generateDataDrivenCoaching(
    ...
    if (opportunities.isNotEmpty) {
      ...
      message +=
          '이번 달 총 ${_formatCurrency(totalSpending)} 지출하셨고, 하루 평균 ${_formatCurrency(dailyAverage)}입니다.';
      ...
      title = '📊 가계부 현황';
      message =
          '이번 달 총 ${_formatCurrency(totalSpending)} 지출하셨고, 하루 평균 ${_formatCurrency(dailyAverage)} 정도 사용하고 계세요. ';
  ...
  /// 카테고리별 구체적인 조언 제공
  List<String> _getSpecificAdviceForCategory(
  ...
  Future<AiCoachingInsight> generateUrgentWarning(
    UserModel user,
    String warning,
  ) async {
    return AiCoachingInsight(
  ...
  Map<String, dynamic> getCoachingStats(UserModel user) {
    ...
    return {
      ...
      'readRate': totalInsights > 0
          ? (totalInsights - unreadCount) / totalInsights
          : 0.0,
    };
  }
