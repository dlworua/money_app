...
-- 사용자는 자신의 결제 내역만 삽입 가능 (서버 측에서만 삽입하도록 제한 권장)
CREATE POLICY "Users can insert own payment transactions"
  ON public.payment_transactions
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- 포인트 결제 내역 테이블 생성 (포인트로 구독 결제 시)
